...
class CocoaLog: NSObject {
    
    //单例
    static let shared = CocoaLog()
    ...
    public func registe(with env: EnvType) {
        ...
        let fileLoggerManagerDefault = LRDDLogFileSub.init(logsDirectory: logDirectory)
        /// file log
...
extension CocoaLog: DDLogFormatter {
...
extension NSObject {
    public func deallocPrint() {
        CocoaLog.debug("deinit: \(NSStringFromClass(type(of: self)))")
    }
}
